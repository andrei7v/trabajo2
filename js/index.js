  ...
  $.getJSON("php/obtenerCards.php", function(response) {
    ...
    if (response.error) {
      ...
      for (var i = 0; i < response.message.length; i++) {
        renderTemplateCards(response.message[i].nombre, response.message[i].descripcion, response.message[i].precio, response.message[i].marca, response.message[i].rutaimagen);
      };
    };
  });
}
